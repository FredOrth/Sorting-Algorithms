...
public class IterativeMergeSortTest {
    ...
    //     assertArrayUsingGetters(sortedArray, testingArray1);
    // }

    @Test
    public void testSortWithDuplicates() {
        StableTestClass[] array = new StableTestClass[] {
                new StableTestClass(4, 2),
                new StableTestClass(5, 1),
                new StableTestClass(4, 3),
                new StableTestClass(5, 4)
        };

        sorter.sort(array);

        StableTestClass[] expected = new StableTestClass[] {
                new StableTestClass(5, 1),
                new StableTestClass(5, 4),
                new StableTestClass(4, 2),
                new StableTestClass(4, 3)
        ...
        assertArrayUsingGetters(expected, array);
    }
    ...
    public void testSortReversed() {
        StableTestClass[] array = new StableTestClass[] {
                new StableTestClass(1, 1),
                new StableTestClass(2, 2),
                new StableTestClass(3, 3)
        };
    ...
    private void assertArrayUsingGetters(StableTestClass[] expected, StableTestClass[] actual) {
        assertTrue("Arrays are not the same length", expected.length == actual.length);

        for (int i = 0; i < expected.length; i++) {
            assertTrue(
                "Mismatch at index " + i + ": expected (comparable=" + expected[i].getComparable() +
                ", tester=" + expected[i].getTester() + ") but was (comparable=" +
                actual[i].getComparable() + ", tester=" + actual[i].getTester() + ")",
                expected[i].getComparable() == actual[i].getComparable()
                        && expected[i].getTester() == actual[i].getTester()
            );
        }
    }
